...
def build_prompt() -> str:
    ...
    # 월요일은 주말 소식 브리핑 (미국 장 휴장)
    if now.weekday() == 0:
        return f"""오늘: {d} ({w}) {t} KST — 월요일 주말 브리핑

미국 주식시장은 주말 동안 휴장이었습니다.
웹 검색으로 주말(토·일) 동안 나온 최신 뉴스를 수집한 후 아래 구조로 한국어 브리핑을 작성하세요.

반드시 검색:
- Fed 인사 주말 발언·성명 (있을 경우)
- 미국 행정부·의회 주요 정책 발표 또는 동향
- 지정학 리스크 (중동·러우·대만·북한 등) 주말 변화
- 주요국 경제지표 발표 (있을 경우)
- 글로벌 중앙은행 (ECB·BoJ·PBoC 등) 발언·결정
- 이번 주 미국 주요 경제지표·실적 발표 일정
- 비트코인·암호화폐 주말 동향
- 유가·금·달러 주말 흐름
- 한국·중국·일본 주말 주요 뉴스
...
[이번 주 시장을 지배할 3~5개 핵심 이슈를 서술형으로. 각 이슈마다 2~3문장. 왜 중요한지, 시장에 어떤 영향을 줄지 해석 포함.]

---

## 🇺🇸 주말 미국 동향

[Fed 발언, 정책 뉴스, 지정학 이슈 등 주말 사이 나온 미국 관련 뉴스. 서술형 문단 3~5개. 각 문단 3~4문장.]

> **핵심 한 줄:** [이번 주 미국 시장의 가장 중요한 변수를 한 문장으로]

---

# 🇰🇷 PART 2 — 한국 시장 주간 전망

[이번 주 한국 시장에 영향을 줄 요인. 수출·반도체·2차전지·자동차·조선 관련 이슈 중심.]
각 이슈마다 한 줄로:
**[영향 강도: High/Medium/Low]** 이슈 요약 — 영향 경로 | 수혜·피해 섹터 | 단기(이번 주) or 중기

---

# 🇨🇳 PART 3 — 중국 주말 동향 → 영향

[주말 중국 관련 뉴스 중 미국·한국 시장에 전이 가능한 것만 선별. 10줄 이내.]
각 이슈마다 한 줄로:
**[High/Medium/Low]** 이슈 요약 — 전이 경로 | 영향 섹터(KR/US) | 단기 or 구조적

---

# 🇯🇵 PART 4 — 일본 주말 동향 → 영향

[주말 일본 관련 뉴스 중 미국·한국 시장에 전이 가능한 것만 선별. 10줄 이내.]
각 이슈마다 한 줄로:
**[High/Medium/Low]** 이슈 요약 — 전이 경로 | 영향 섹터(KR/US) | 단기 or 구조적
...
**▲ 급등**
각 종목을 서술형으로: "종목명(티커) +X% — 이유 한 줄" 형식. 5개.
...
- 핀테크·미디어·방산 등 테마 흐름이 보이면 테마 맥락도 포함
...
[전일 미국 증시 마감 이후 이슈 중 한국 시장에 전이 가능한 것만 선별]
[10줄 이내로 압축. 사실 정보만. 추측 금지]

선별 기준 (하나 이상 충족 시 포함):
1. 한국 수출/반도체/2차전지/자동차/조선 등 주요 산업에 직접 영향
2. 달러인덱스·미 국채금리·유가·구리·원자재 급변
3. 글로벌 리스크온/리스크오프 전환 신호
4. 대형 미국 기업 실적이 한국 공급망에 영향
5. 정책·지정학 리스크가 아시아 시장에 전이 가능

각 이슈마다 한 줄로:
**[영향 강도: High/Medium/Low]** 이슈 요약 — 영향 경로(금리/환율/실적/밸류에이션/심리) | 수혜·피해 섹터 | 단기(1~3일) or 중기(1~4주)

---

# 🇨🇳 PART 3 — China Overnight → US/Korea Impact

[전일 15:00 KST ~ 금일 06:50 KST 사이 중국 관련 이벤트 중 미국·한국 시장에 전이 가능한 것만 선별]
[10줄 이내 압축. 검증 가능한 데이터·공식 발표만. 해설·추측 보도 제외]

대상: 매크로, 정책, 유동성, 부동산, 테크, 지정학
각 이슈마다 한 줄로:
**[High/Medium/Low]** 이슈 요약 — 전이 경로(금리/환율/공급망/수요/심리) | 영향 섹터(KR/US) | 단기(1~3일) or 구조적(중장기)

---

# 🇯🇵 PART 4 — Japan Overnight → US/Korea Impact

[전일 15:00 KST ~ 금일 06:50 KST 사이 일본 관련 이벤트 중 미국·한국 시장에 전이 가능한 것만 선별]
[10줄 이내 압축. 검증 가능한 데이터·공식 발표만. 해설·추측 보도 제외]

대상: BoJ 정책, 엔화, 수출기업 실적, 반도체·자동차 경쟁, 지정학
각 이슈마다 한 줄로:
**[High/Medium/Low]** 이슈 요약 — 전이 경로(금리/환율/경쟁/공급망/심리) | 영향 섹터(KR/US) | 단기(1~3일) or 구조적(중장기)
